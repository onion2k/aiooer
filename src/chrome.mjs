...
function partsPanel(parts, current) {
  const items = parts
    .map((p) => {
      ...
      return `<li class="parts-item${here ? ' is-current' : ''}"><a class="parts-link" href="Part${p.n}.dc.html"${here ? ' aria-current="page"' : ''}><span class="parts-num">Part ${p.n}</span><span class="parts-title">${esc(p.shortTitle)}</span></a><span class="parts-time">${esc(p.time)}${here ? '<span class="parts-here"> · You are here</span>' : ''}</span></li>`;
    })
    .join('');
  const introHere = current === 0;
  return `<nav class="panel parts-panel" id="parts-panel" aria-label="Course parts" hidden="{{parts.hidden}}" onKeyDown="{{parts.onKeyDown}}"><div class="shell panel-inner"><p class="parts-intro"><a class="parts-home" href="Main.dc.html"${introHere ? ' aria-current="page"' : ''}>${ICONS.book}<span>Course introduction</span></a></p><ol class="parts-list" role="list">${items}</ol><div class="panel-actions"><button type="button" class="btn-line" onClick="{{parts.close}}">${ICONS.close}<span>Close the list of parts</span></button></div></div></nav>`;
}

export function header(parts, current) {
  const homeCurrent = current === 0 ? ' aria-current="page"' : '';
  return `<a class="skip-link" href="#main">Skip to main content</a><header class="site-header"><div class="shell header-bar"><a class="wordmark" href="Main.dc.html"${homeCurrent}>${COURSE_TITLE}</a><div class="header-actions"><button type="button" class="header-btn" aria-expanded="{{parts.expanded}}" aria-controls="parts-panel" onClick="{{parts.toggle}}" ref="{{parts.buttonRef}}">${ICONS.parts}<span>Parts</span></button><button type="button" class="header-btn" aria-expanded="{{settingsPanel.expanded}}" aria-controls="settings-panel" onClick="{{settingsPanel.toggle}}" ref="{{settingsPanel.buttonRef}}">${ICONS.settings}<span>Reading settings</span></button></div></div>${partsPanel(parts, current)}${settingsPanel()}</header>`;
}
...
export function toc(sections) {
  const items = sections
    .map((s) => {
      const num =
        s.number !== null
          ? `<span class="toc-num" aria-hidden="true">${s.number}</span>`
    ...
    .join('');
  return `<nav class="toc" aria-labelledby="toc-title"><h2 class="toc-title" id="toc-title"><span class="toc-static">On this page</span><button type="button" class="toc-toggle" aria-expanded="{{toc.expanded}}" aria-controls="toc-list" onClick="{{toc.toggle}}"><span>On this page</span>${ICONS.chevron}</button></h2><ol class="toc-list" id="toc-list" role="list" hidden="{{toc.hidden}}">${items}</ol></nav>`;
}

export function crumbs(n, shortTitle) {
  return `<nav class="crumbs" aria-label="Breadcrumb"><ol role="list"><li><a href="Main.dc.html">Course introduction</a><span class="crumb-sep" aria-hidden="true">/</span></li><li><a href="Part${n}.dc.html" aria-current="page">Part ${n}: ${esc(shortTitle)}</a></li></ol></nav>`;
}
...
export function pager(n, parts) {
  ...
  return `<nav class="pager" aria-label="Previous and next"><a class="pager-link is-prev" href="${prev.href}"><span class="pager-dir">${ICONS.arrowLeft}<span>Previous</span></span><span class="pager-title">${esc(prev.label)}</span></a><a class="pager-link is-next" href="${next.href}"><span class="pager-dir"><span>${next.dir || 'Next'}</span>${ICONS.arrowRight}</span><span class="pager-title">${esc(next.label)}</span></a></nav>`;
}
...
export function footer(parts, currencyNote) {
  const links = [`<li><a href="Main.dc.html">Course introduction</a></li>`]
    ...
    .join('');
  return `<footer class="site-footer"><div class="shell"><p class="footer-title">${COURSE_TITLE}</p><nav aria-label="All pages"><ul class="footer-links" role="list">${links}</ul></nav><p class="footer-note">${currencyNote}</p></div></footer>`;
}
