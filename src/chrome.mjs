...
function partsPanel(modules, current) {
  const item = (p) => {
    ...
    return `<li class="parts-item${here ? ' is-current' : ''}"><a class="parts-link" href="page:${p.out}"${here ? ' aria-current="page"' : ''}><span class="parts-num label">Part ${p.n}</span><span class="parts-title">${esc(p.shortTitle)}</span></a><span class="parts-time label">${esc(p.time)}${here ? '<span class="parts-here"> · You are here</span>' : ''}</span></li>`;
  };
...
export function toc(sections, part) {
  const items = sections
    ...
    .join('');
  // The contents are sticky, so once the title has scrolled away this is the
  // only thing on screen that says where the reader is. It names the module
  // and the part rather than "On this page", which said nothing a reader did
  // not already know. "Contents" is kept for a screen reader, which hears the
  // heading as the list's name and would otherwise be told a part's title
  // twice over, once for the page and once for its contents.
  const where = `<span class="toc-where label">${esc(part.module)} · Part&#160;${part.n}</span><span class="toc-page">${esc(
    part.shortTitle,
  )}</span><span class="sr-only">: contents</span>`;
  return `<nav class="toc" aria-labelledby="toc-title"><h2 class="toc-title" id="toc-title"><span class="toc-static">${where}</span><button type="button" class="toc-toggle" aria-expanded="{{toc.expanded}}" aria-controls="toc-list" onClick="{{toc.toggle}}"><span>${where}</span>${ICONS.chevron}</button></h2><ol class="toc-list" id="toc-list" role="list" hidden="{{toc.hidden}}">${items}</ol></nav>`;
}
    ...
    .map(
      (m) =>
        `<div class="footer-group" style="--hue: var(--hue-${m.hue})"><p class="footer-group-title label" id="footer-group-${m.slug}">${esc(m.name)}</p><ul class="footer-links" role="list" aria-labelledby="footer-group-${m.slug}">${m.parts
          .filter((p) => p.written)
          .map((p) => `<li><a href="page:${p.out}">Part ${p.n}: ${esc(p.shortTitle)}</a></li>`)
          .join('')}</ul></div>`,
    )
    .join('');
