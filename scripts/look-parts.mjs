...
// test-results/parts.
//   node scripts/look-parts.mjs <width> <File.dc.html> <selector>[@index] ...
...
if (!file) {
  console.error('usage: node scripts/look-parts.mjs <width> <File.dc.html> <selector>[@index] ...');
  process.exit(2);
}
...
for (const spec of selectors) {
  ...
  const name = `${file.replace('.dc.html', '')}-${width}-${++n}.png`;
